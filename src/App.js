...
class App extends Component {
  ...
  render() {
    const {isTrue, clickedTabId, score, time, imgUrl} = this.state
    const newImageList = imagesList.filter(
      eachUrl => eachUrl.category === clickedTabId,
    )
              ...
              <ul className="tabs-lists">
                {tabsList.map(eachTabs => (
                  <li
                    id={eachTabs.tabId}
                    key={eachTabs.tabId}
                    onClick={() => this.onClickTab(eachTabs.tabId)}
                  >
                    <button
                      type="button"
                      className={`tab-button ${
                        clickedTabId === eachTabs.tabId ? 'clicked-tab' : ''
                      }`}
                    >
                      {eachTabs.displayText}
                    </button>
                  </li>
                ))}
              </ul>
              <ul className="thumbnail-url-lists">
                {newImageList.map(eachUrl => (
                  <li key={eachUrl.id}>
                    <button
                      className="thumbnail-btn"
                      type="button"
                      onClick={() =>
                        this.onClickThumbnail(eachUrl.thumbnailUrl)
                      }
                    >
                      <img
                        src={eachUrl.thumbnailUrl}
                        alt="thumbnail"
